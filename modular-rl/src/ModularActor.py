...
class ActorGraphPolicy(nn.Module):
    ...
    def __init__(
        ...
        if self.bu:
            ...
            if self.td:
                self.sNet = nn.ModuleList(
                    [ActorUp(state_dim, msg_dim, max_children)] * self.num_limbs
                ).to(device)
            # bottom-up only
            else:
                self.sNet = nn.ModuleList(
                    [
                        ActorUpAction(
                            state_dim, msg_dim, max_children, action_dim, max_action
                        )
                    ]
                    * self.num_limbs
                ).to(device)
        ...
        if self.td:
            ...
            if self.bu:
                self.actor = nn.ModuleList(
                    [
                        ActorDownAction(
                            msg_dim, action_dim, msg_dim, max_action, max_children
                        )
                    ]
                    * self.num_limbs
                ).to(device)
            # top-down only
                ...
                self.actor = nn.ModuleList(
                    [
                        ActorDownAction(
                            state_dim, action_dim, msg_dim, max_action, max_children
                        )
                    ]
                    * self.num_limbs
                ).to(device)
        ...
        if not self.bu and not self.td:
            self.actor = nn.ModuleList(
                [ActorVanilla(state_dim, action_dim, max_action)] * self.num_limbs
            ).to(device)
            if not self.disable_fold:
                for i in range(self.num_limbs):
                    setattr(self, "actor" + str(i).zfill(3), self.actor[i])
    ...
    def forward(self, state, mode="train"):
        ...
        if not self.bu and not self.td:
            for i in range(self.num_limbs):
                if not self.disable_fold:
                    self.action[i] = self.fold.add(
                        "actor" + str(0).zfill(3), self.input_state[i]
                    )
                else:
        ...
        if not self.disable_fold:
            if self.td:
                self.msg_up[node] = self.fold.add(
                    "sNet" + str(0).zfill(3), state, *msg_in
                )
            else:
                self.msg_up[node], self.action[node] = self.fold.add(
                    "sNet" + str(0).zfill(3), state, *msg_in
                ).split(2)
        else:
            if self.td:
    ...
    def top_down_transmission(self, node):
        ...
        if not self.disable_fold:
            self.action[node], self.msg_down[node] = self.fold.add(
                "actor" + str(0).zfill(3), state, msg_in
            ).split(2)
        else:
