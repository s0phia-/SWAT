...
class CriticGraphPolicy(nn.Module):
    ...
    def __init__(
        ...
        if self.bu:
            ...
            if self.td:
                self.sNet = nn.ModuleList(
                    [CriticUp(state_dim, action_dim, msg_dim, max_children)]
                    * self.num_limbs
                ).to(device)
            # bottom-up only
            else:
                self.sNet = nn.ModuleList(
                    [CriticUpAction(state_dim, action_dim, msg_dim, max_children)]
                    * self.num_limbs
                ).to(device)
        ...
        if self.td:
            ...
            if self.bu:
                self.critic = nn.ModuleList(
                    [CriticDownAction(msg_dim, action_dim, msg_dim, max_children)]
                    * self.num_limbs
                ).to(device)
            # top-down only
            else:
                self.critic = nn.ModuleList(
                    [CriticDownAction(state_dim, action_dim, msg_dim, max_children)]
                    * self.num_limbs
                ).to(device)
            if not self.disable_fold:
                for i in range(self.num_limbs):
        ...
        if not self.bu and not self.td:
            self.critic = nn.ModuleList(
                [CriticVanilla(state_dim, action_dim)] * self.num_limbs
            ).to(device)
            if not self.disable_fold:
                for i in range(self.num_limbs):
                    setattr(self, "critic" + str(i).zfill(3), self.critic[i])
    ...
    def bottom_up_transmission(self, node):
        ...
        if not self.disable_fold:
            if self.td:
                self.msg_up[node] = self.fold.add(
                    "sNet" + str(0).zfill(3), state, action, *msg_in
                )
            else:
                self.msg_up[node], self.x1, self.x2 = self.fold.add(
                    "sNet" + str(0).zfill(3), state, action, *msg_in
                ).split(3)
        else:
            if self.td:
                ...
                self.msg_up[node], self.x1, self.x2 = self.sNet[node](
                    state, action, *msg_in
                )
    ...
    def top_down_transmission(self, node):
        ...
        if not self.disable_fold:
            self.x1[node], self.x2[node], self.msg_down[node] = self.fold.add(
                "critic" + str(0).zfill(3), state, action, msg_in
            ).split(3)
        else:
            self.x1[node], self.x2[node], self.msg_down[node] = self.critic[node](
                state, action, msg_in
            )
    ...
    def zero_func_td(self):
        return torch.zeros(
            (1, self.batch_size, self.msg_dim * self.max_children), requires_grad=True
        ).to(device)

    def zero_func_bu(self):
        return torch.zeros((1, self.batch_size, self.msg_dim), requires_grad=True).to(
            device
        )
