...
def get_args():
    ...
    parser.add_argument(
        "--lr", default=0.0001, type=float, help="learning rate for Adam"
    )
    parser.add_argument(
        "--max_episode_steps",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=5e5,
        help="maximum replay buffer size for each morphology",
    )
    parser.add_argument(
        "--max_children",
        type=int,
    ...
    parser.add_argument(
        "--transformer_norm", default=1, type=int, help="Use layernorm",
    )
    ...
    if args.expID is None:
        args.expID = str(running_start_time.strftime("%m_%d-%X")) + '_' + args.label
    else:
        import json
        import os
        ...
        print('no args.txt', args.__dict__)
