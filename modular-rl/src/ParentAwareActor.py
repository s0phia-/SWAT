...
class TransformerModel(nn.Module):
    def __init__(
        ...
        ninp_dec += output_size  # parent conditioning
    ...
    def encode(self, src, graph):
        """Per-node transformer features, before parent-conditioning/decoding: [N, B, ninp_dec]."""
    ...
    def forward(self, src, graph=None):
        # this node's action is itself the parent's decoder output, so nodes must
        # be decoded in traversal order: graph['parents'] is built pre-order, so
        # a node's parent always has a strictly lower index and is already decoded
        ...
        for i, parent_idx in enumerate(parents):
            parent_action = actions[parent_idx] if parent_idx >= 0 else zero_action
            dec_in = torch.cat([output[i], parent_action], dim=1)
            actions.append(self.decoder(dec_in))
